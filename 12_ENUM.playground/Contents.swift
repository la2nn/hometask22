...
var arrayOfFigures = [whiteKing, blackKing, blackPawn, whiteRook1, whiteRook2, whiteKnight]
...
func possibleMove(_ startPos: (String, Int), endPos: (String, Int),_ closure: (Int, Int) -> Bool) -> Bool {
...
func moveFigure(_ newFigure: inout Chessman, newPos: (x: Chess.Horizontal, y: Chess.Vertical)) {
    ...
    case (.King, let color, let horizontal, let vertical) where
        possibleMove((horizontal.rawValue, vertical.rawValue), endPos:(newPos.0.rawValue, newPos.1.rawValue)) {($0 == 1 && $1 == 0) || ($0 == 0 && $1 == 1) || ($0 == 1 && $1 == 1)}:
        newFigure = (.King, color, newPos.0, newPos.1)
        
    case (.Queen, let color, let horizontal, let vertical) where
        possibleMove((horizontal.rawValue, vertical.rawValue), endPos:(newPos.0.rawValue, newPos.1.rawValue)) {($0 == 0 || $1 == 0 || $0 == $1)}:
        newFigure = (.Queen, color, newPos.0, newPos.1)
        
    case (.Rook, let color, let horizontal, let vertical) where
        possibleMove((horizontal.rawValue, vertical.rawValue), endPos:(newPos.0.rawValue, newPos.1.rawValue)) {($0 == 0 || $1 == 0)}:
        newFigure = (.Rook, color, newPos.0, newPos.1)
        
    case (.Bishop, let color, let horizontal, let vertical) where
        possibleMove((horizontal.rawValue, vertical.rawValue), endPos:(newPos.0.rawValue, newPos.1.rawValue)) {($0 == $1)}:
        newFigure = (.Bishop, color, newPos.0, newPos.1)
        
    case (.Knight, let color, let horizontal, let vertical) where
        possibleMove((horizontal.rawValue, vertical.rawValue), endPos:(newPos.0.rawValue, newPos.1.rawValue)) {($0 == 1 && $1 == 2) || ($0 == 2 && $1 == 1)}:
        newFigure = (.Knight, color, newPos.0, newPos.1)
        
    case (.Pawn, let color, let horizontal, let vertical) where
        possibleMove((horizontal.rawValue, vertical.rawValue), endPos:(newPos.0.rawValue, newPos.1.rawValue)) {($0 == 0 && $1 == 1)}:
        newFigure = (.Pawn, color, newPos.0, newPos.1)
        
    case (let type, let color, _, _):
        print("Нельзя переметстить фигуру \(type) \(color) на клетку \(newPos.0.rawValue, newPos.1.rawValue)")
        break
    }
}

moveFigure(&arrayOfFigures[0], newPos: (x: .F, y: .Eight))
moveFigure(&arrayOfFigures[5], newPos: (x: .E, y: .Two))
