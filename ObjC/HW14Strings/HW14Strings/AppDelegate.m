...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    // 2
    NSLog(@"FIRSH HALF: %@", [text substringToIndex:[text length] / 2]);
    NSLog(@"\n SECOND HALF: %@", [text substringFromIndex:[text length] / 2]);
   
    //3
    NSArray *sentences = [text componentsSeparatedByString: @"."];
    NSLog(@"%@, %lu", sentences, sentences.count);
    
    //4 + 5
    NSMutableArray *sentencesWithoutNSStrings = [[NSMutableArray alloc] init];
    NSMutableArray *reversedOrderOfWordsInSentences = [[NSMutableArray alloc] init];
    
    for (NSString *sentence in sentences) {
        NSRange range = [sentence rangeOfString: @"NSString"];
        
        if (range.location != NSNotFound) {
            [sentencesWithoutNSStrings addObject: sentence];
        }
        
        NSArray *words = [sentence componentsSeparatedByString: @" "];
        [reversedOrderOfWordsInSentences addObject: [[[words reverseObjectEnumerator] allObjects] componentsJoinedByString: @" "]];
    }
    
    NSLog(@"%@, %lu", sentencesWithoutNSStrings, sentencesWithoutNSStrings.count);
    NSLog(@"%@", reversedOrderOfWordsInSentences);
   
    //6
    for (int i = 0; i < sentences.count - 1; i++ ) {                                // юзаю так, потому что последняя строка "" и вылазит ошибка ( !(isEqual @"") тоже пытался )
        if ([[sentences[i] substringToIndex:1] isEqual: @" "]) {
            NSMutableArray *tmp = [[NSMutableArray alloc] init];
            [tmp addObject: [sentences[i] substringFromIndex: 1]];
            
            for (NSString *letters in tmp) {
                NSRange start = NSMakeRange(0, 1);
                NSMutableString *result = [letters stringByReplacingCharactersInRange:start withString: [letters substringToIndex:1].lowercaseString];
                NSRange end = NSMakeRange(result.length - 1, 1);
                result = [result stringByReplacingCharactersInRange:end withString: [letters substringFromIndex: letters.length - 1].uppercaseString];
                NSLog(@"%@", result);
            }
        }
    }
